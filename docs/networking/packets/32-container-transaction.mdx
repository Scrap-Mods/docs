...
| 0x1B   | Unknown      | u8         | Not known to be anything other than `0x80`. Could possibly be related to the `mustCollectAll` parameter of the `sm.container.collect` Lua API function. Its description reads: <blockquote>Must collect all items for the transaction to be valid. Defaults to true.</blockquote> |

#### Action 3 - Spend

Calls `ContainerManager2.spend()` under the hood, but has never been seen.
...
| Offset | Field Name | Field Type | Notes |
|--------|------------|------------|-------|
| 0x01   | ???        | ???        | To be documented. |

#### Action 4 - Collect To Slot (?)

Calls `ContainerManager2.collectToSlot()` under the hood, but has never been seen.
