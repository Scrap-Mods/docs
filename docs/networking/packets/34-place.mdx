...
| 0x14   | Local Position Start | [Vec3i](#vec3i) | The position, local to the body, where the shape is placed on. The placed shape will be connected to all sticky faces adjacent to this position. |
| 0x20   | Z Axis               | [Vec3i](#vec3i) | The Z-axis of the placed shape. |
| 0x2C   | X Axis               | [Vec3i](#vec3i) | The X-axis of the placed shape. |
| 0x38   | Local Position       | [Vec3i](#vec3i) | The position, local to the body, of the placed shape. This value matches the value returned by the `shape:getLocalPosition()` Lua API function. |
